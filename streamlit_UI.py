...
def run_yolo_webcam():
    ...
    # Release the webcam capture object and close the window when done
    cap.release()
...
def run(
    ...
    stop_webcam_button = st.button("Stop Webcam")
    frame_placeholder = st.empty()
    ...
    for det in pred:
        if len(det):
            for *xyxy, conf, cls in det:
                results.append((xyxy, conf, int(cls), names[int(cls)]))
    return results
...
modelType = st.selectbox('Select the model', ('None', 'Yolov8', 'Yolov9'))
